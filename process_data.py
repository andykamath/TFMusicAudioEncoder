...
from tensorflow import audio as audio_ops
...
def save_to_wav(audio_arr_ch1, audio_arr_ch2, sample_rate, original_song_ch1, original_song_ch2, epoch, folder, sess, verbose=False):
        ...
        plt.savefig(str(folder) + '/compressed' + str(epoch) + '.png')
        plt.clf()       
        ...
        wav_encoder_orig = audio_ops.encode_wav(
                original_song, sample_rate)

        wav_file = sess.run(wav_encoder)
        wav_orig = sess.run(wav_encoder_orig)
        open(str(folder)+f'/out.wav{epoch}', 'wb').write(wav_file)
        open(str(folder)+f'/wav_orig{epoch}.wav', 'wb').write(wav_orig)
